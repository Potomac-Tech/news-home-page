...
import type { SponsorUnitData } from "../data/newsIntelligence";

type SponsorUnitProps = {
    unit: SponsorUnitData;
};

const CreativeFrame = ({ unit }: { unit: SponsorUnitData }) => {
    if (unit.creativeUrl) {
        return (
            <img
...
const SponsorUnit: React.FC<SponsorUnitProps> = ({ unit }) => {
    const content = (
        <>
    ...
    if (unit.sponsorWebsiteUrl) {
        return (
            <a
                ...
                className="glass-card block rounded p-5 transition hover:border-potomac-gold/55 hover:bg-white/[0.04]"
            >
                {content}
            </a>
        );
    ...
    return <aside className="glass-card rounded p-5">{content}</aside>;
};
