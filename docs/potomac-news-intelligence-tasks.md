...
- [x] Task 108: Verify canonical remote Supabase migrations and role journeys
  ...
  - Non-technical summary: Production now has dedicated, uniquely credentialed QA identities for Explorer, Scout, Command/organization admin, editor, analyst, and admin. Each persona can sign in and receives the intended access without restoring the unsafe shared-password seed. Paid, member, organization, editorial, analyst, and administrator boundaries were exercised across all 17 required data areas, and temporary test content was removed afterward.
  - Verification: Confirmed 73 matching named local/remote migrations plus five older unnamed production records. Verified zero remaining deterministic seed users and zero remaining seed organizations. Created six canonical Auth QA identities with approved/completed profiles, normalized role assignments, a non-billable QA Command organization, and time-limited Scout/Command entitlements. All six Auth password grants succeeded. Completed 102 authenticated Data API read checks across the 17 representative RLS tables with no failures and 25 allowed/denied write checks covering every required area with all expected outcomes. Final cleanup returned zero temporary content rows. QA sessions were revoked and in-memory credentials were cleared. `npm test` passed all 104 tests. Detailed non-secret results are in `docs/supabase-production-verification.md`; no QA credentials or test secrets were stored in the repository.
  - Blocked reason: None.
...
- [x] Task 113: Add Meridian, Resend, and operational release QA and monitoring checks
  ...
  - Non-technical summary: Automated safeguards now stop releases when Meridian payment rules, server email routing, Resend Free limits, sender/domain verification, duplicate handling, or delayed-delivery protections are missing. The operations guide now covers resets, retries, capped delivery, and future upgrade approval.
  - Verification: `npm run test:email-operations` passed 21 scenarios with a zero-issue audit; `npm test` passed 123 tests; lint and production build passed. Canonical Supabase checks confirmed Free-plan caps of 90/100 daily and 2,700/3,000 monthly, 10/300 operational reserves, inbound disabled, lead/audit persistence, and no automatic Command entitlement or role grant. Resend now has one verified apex `potomacdb.com` sending domain and one domain-scoped `Cabeus Production Apex` key; the obsolete `Supabase Integration` key was deleted. Supabase Auth delivered a production magic link from `no-reply@potomacdb.com` through canonical project `xlpkdoeldtlhearqajat`, and the callback session remained active across protected navigation after direct response-cookie persistence and logout-link prefetch fixes. A live Meridian inquiry created lead `3e36fc82-ed10-4d03-9089-8b353f280fd2`, delivery event `fcdb95b9-cd85-4c02-b630-f6b3d751cda5`, and delivered Resend message `69855c71-0ac0-4594-8eb6-a329ad260943` from/to `info@potomacdb.com` with Reply-To `jake@potomacdb.com`; the corresponding account retained zero active Command entitlements and roles. The three obsolete `auth.potomacdb.com` MX/TXT records were removed, while apex MX, DKIM, and SPF records resolve through `1.1.1.1`. Cloudflare version `d0ca9ca7-7806-4f06-9201-da6b930b6829` deployed successfully, and the production crawler visited 60 routes with no console, network, CSP, runtime, or routing issues.
  - Blocked reason: None.
...
- [x] Task 117: Add production Nexus access for all approved membership tiers
  ...
  - Superseded by: None.
  - Goal: Let Explorer, Scout, and Meridian members open Nexus with the same identity and the correct Nexus role.
  - Acceptance criteria: The member workspace and `/nexus` route offer a protected handoff to `https://nexus-explore.potomacdb.com/0auth`; verified/profile-complete users require an active normalized membership role; Explorer maps to `base_user`, Scout maps to `premium_user`, and Meridian/Command maps to `superior_user`; Nexus administrators remain administrators; role synchronization responds to member profile, role-assignment, and entitlement changes; members cannot update their own Nexus role; the Nexus client permits `base_user` only when an active Cabeus member role exists; the one-time handoff exposes no service credential or reusable session token.
...
- [ ] Task 121: Run and sign the July 21 production release
  ...
  - Non-technical summary: The release review remains a NO-GO, although the lunar launch feed, all ten stock prices, Contract Awards tracker, and Resend Free email operations are current and healthy. Two official NASA lunar awards are published, six unrelated NIH awards were removed, and the remaining blocker is one named technical approval.
  - Verification: Lint passed; 155/155 unit and integration tests passed; 21/21 email-operation tests passed; the production build and all 6 E2E journeys passed. Migration `review_july_contract_awards` published two cited NASA lunar awards and archived six NIH workforce false positives with review decisions. Regression coverage prevents Gateway-only records without NASA or explicit space context. The post-deploy production release audit reported zero issues across 24 routes, and the 52-destination production crawl reported zero issues on Cloudflare version `fcf715c0-a54a-454b-adae-7c55ac047c70`. The latest launch run and both Alpha Vantage batches remain healthy, and Task 108 supplies current named-role evidence. On July 27, 2026, release owner Jacob Matthews directed resolution of the Resend blocker and the live dashboard review reconfirmed one verified `potomacdb.com` domain, the Free plan, paid overage unavailable, sending-only production keys, and delivered Supabase Auth magic-link, password-reset, and signup messages; the manual retry and capped-delivery runbook review is now signed.
  - Blocked reason: NO-GO until a named technical release owner records approval. Reconfirmed by the 2026-07-28 15:24 EDT automation run; no technical approver or approval was supplied.
