...
- [ ] Task 079: Build `/request-access` Sign In / Sign Up auth page with free Explorer default
  - Priority: P0
  ...
  - Non-technical summary: The unified Sign In / Sign Up gateway is implemented with free Explorer selected by default, a verification-aware signup application, sign-in, password reset, return-context preservation, and legacy entry redirects. The post-verification profile-completion handoff is pending the next profile task.
  - Verification: `npm run lint` passed; `npm test` passed with 11 tests; `npm run build` passed. Local rendered QA at `http://127.0.0.1:3001/request-access` confirmed Sign Up is selected by default, Explorer is the selected free membership, and the signup form accepts any email domain; switching to Sign In exposes magic-link, password, and reset-password paths. Local route checks confirmed `/apply` redirects to `/request-access` and `/auth/login?next=/news` redirects to `/request-access?tab=signin&next=%2Fnews`. Cloudflare deployment `f2eb01e6-7861-46ef-bba5-6c9e630774ae` succeeded; production `/request-access` returned `200`, `/apply` redirected to `/request-access`, and `/member` returned the expected `307` login redirect. Full live signup, verification-email delivery, callback completion, and password-reset delivery were not exercised because those actions send email and require a test mailbox.
  - Blocked reason: Cannot satisfy the required verified-but-profile-incomplete redirect until Task 080 creates `/account/profile/complete` and the normalized profile-completion gate. Task 079 remains unchecked so the next run resumes at that dependency rather than treating the auth flow as fully complete.
...
- [ ] Task 080: Build profile completion flow and profile-complete gate
  ...
  - Blocked reason: Depends on member profile schema from Task 007 and Supabase Auth test users from Task 108 for live verification; fixture-based UI and helper tests can proceed earlier.
