...
- [ ] Task 116: Add lunar time display to the main page
  ...
  - Verification: Source review completed on 2026-07-15 against BIPM/CIPM decisions, NASA's Coordinated Lunar Time program, and Bourgoin, Defraigne, and Meynadier, *Lunar reference timescale*, Metrologia 63 (2026) 015003, DOI `10.1088/1681-7575/ae2c03`. The paper recommends `TL = TCL` but documents multiple candidate realizations and no adopted UTC offset; BIPM Decision CIPM/115-6 still describes the international lunar reference definition as Draft Resolution D for the 28th CGPM.
  - Blocked reason: No internationally adopted, operational UTC-to-lunar-reference realization is published yet. Displaying the paper's mean `TCL - TT` rate of about `58.7 microseconds/day` would still require Potomac to invent an epoch and constant offset, violating the requirement not to fabricate a lunar timezone. Resume after the 28th CGPM adopts conventional values and a traceable realization, or after explicit product approval to label a separately scoped display as a provisional research estimate rather than current `Lunar Time`.
