...
- [ ] Task 121: Run and sign the July 21 production release
  ...
  - Non-technical summary: The release review remains a NO-GO, although the lunar launch feed, all ten stock prices, and the Contract Awards tracker are now current and healthy. Two official NASA lunar awards are published, and six unrelated NIH awards were removed. The remaining blockers are human release approvals.
  - Verification: Lint passed; 155/155 unit and integration tests passed; 21/21 email-operation tests passed; the production build and all 6 E2E journeys passed. Migration `review_july_contract_awards` published two cited NASA lunar awards and archived six NIH workforce false positives with review decisions. Regression coverage prevents Gateway-only records without NASA or explicit space context. The post-deploy production release audit reported zero issues across 24 routes, and the 52-destination production crawl reported zero issues on Cloudflare version `fcf715c0-a54a-454b-adae-7c55ac047c70`. The latest launch run and both Alpha Vantage batches remain healthy, and Task 108 supplies current named-role evidence.
  - Blocked reason: NO-GO until the release owner signs the Resend manual retry procedure review and a named technical release owner records approval.
