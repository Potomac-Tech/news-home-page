...
- [x] Task 038: Add tier-based dataset release states and one-year exclusivity logic
  ...
  - Verification: Added `supabase/migrations/20260629131559_dataset_release_states.sql` with the `dataset_catalog_release_state` enum, release timing columns, one-year Command-exclusive constraint, unavailable-reason constraint, seeded release states, and an unavailable Potomac polar-volatiles placeholder. Updated `/datasets` and the catalog loader to render Command-exclusive, Scout-delayed, public/demo, and unavailable states with exclusive windows, Scout release dates, public/demo release dates, and unavailable reasons. `npm run build:next` passed; `npm run build` passed; `git diff --check` passed with recurring LF-to-CRLF warnings on touched files. Static checks confirmed the release enum, one-year constraint, release timing fields, four release states, and unavailable placeholder. Local route check on `http://127.0.0.1:3001/datasets` confirmed Command-exclusive, Scout-delayed, Public/demo, Unavailable, Exclusive until, Scout release, and the polar-volatiles placeholder render. `npx supabase migration list --local --workdir "C:\Users\JacobMatthews\Documents\Potomac Website"` could not connect because no local Supabase Postgres was listening on `127.0.0.1:54322`. `npm run lint` still cannot run because the repo has no ESLint configuration file. Live Supabase catalog reads and remote migration application were not exercised because no Supabase publishable key, authenticated database tooling, or reachable applied schema was available.
  - Blocked reason: None.
