...
- [ ] Task 113: Add Meridian, Resend, and operational release QA and monitoring checks
  ...
  - Verification: `npm run test:email-operations` passed 21 scenarios and produced a zero-issue audit; `npm test` passed 123 tests; lint and production build passed. Canonical Supabase checks confirmed Free-plan caps of 90/100 daily and 2,700/3,000 monthly, 10/300 operational reserves, inbound disabled, one sending domain, and no current quota/event rows. Cloudflare now lists all six required encrypted Resend secrets, including explicit Free-plan guards. Live Resend inspection confirmed the Free transactional plan (3,000 emails, $0/month, no payment method), one verified `auth.potomacdb.com` domain, and a delivered Supabase Auth magic-link email whose callback targets canonical project `xlpkdoeldtlhearqajat` and `/auth/callback?next=%2Fmember`. Cloudflare deployment `8daca7fd-6a09-436f-bd29-24f1b0a7fdf0` succeeded, and the production crawler visited 60 routes with no console, network, CSP, or routing issues.
  - Blocked reason: Resend verifies only `auth.potomacdb.com`, which authorizes `no-reply@auth.potomacdb.com` but not the required operational sender `info@potomacdb.com`. Complete this task by verifying the apex `potomacdb.com` sending domain and reconciling the one-domain rule without interrupting Supabase Auth delivery, then perform one authorized operational inquiry send and confirm its provider ID, Reply-To, lead/audit row, and delivery status. The configured Resend MCP API key is also rejected and should be rotated before connector-based checks resume.
