# Scout/Command Developer Platform
...
`developer_tier_limits` defines Scout, Command, and staff limits for monthly API
quota, daily exports, active API keys, webhook subscriptions, Command endpoints,
and usage retention.
...
Command webhook subscriptions, event selection, delivery state, retries, and
failure tracking.
...
- Scout, Command, editor, analyst, and admin roles can use developer-platform
  records.
- Webhook writes are limited to Command and staff roles.
...
- recent usage logs
- webhook subscriptions and Command-only gating
- recent export jobs
