...
declare
    ...
    v_request_id bigint;
begin
    if p_job not in ('launches', 'space-weather') then
...
declare
    v_job_id bigint;
begin
    select jobid into v_job_id from cron.job where jobname = 'ingest-launch-library-2';
    ...
    select jobid into v_job_id from cron.job where jobname = 'ingest-noaa-space-weather';
    if v_job_id is not null then perform cron.unschedule(v_job_id); end if;
    perform cron.schedule(
        'ingest-noaa-space-weather',
        '*/15 * * * *',
        $job$select private.invoke_production_tracker_ingestion('space-weather');$job$
    );
end;
$$;
