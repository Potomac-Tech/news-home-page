            ...
            {rows.length ? <div className="grid gap-4">
                {rows.map((row) => { const times = timeLabel(row.scheduledAt, timeZone); return <article key={row.id} className="border border-potomac-gold/25 bg-potomac-primary/70 p-5 md:p-6">
                    ...
                    <dl className="mt-6 grid gap-x-6 gap-y-4 border-t border-white/10 pt-5 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="min-w-0"><dt className="font-mono text-[0.65rem] font-bold uppercase text-potomac-gold">Value state</dt><dd className="mt-1 break-words text-sm text-potomac-cream/80">{row.value ? <>{row.value.state === "withheld" ? "not disclosed" : row.value.state.replaceAll("_", " ")} / {money(row)}{row.value.state === "analyst_estimate" && row.value.methodology ? <span className="mt-2 block text-xs leading-5 text-potomac-cream/60">Methodology: {row.value.methodology} / Confidence: {row.value.confidence ?? "pending"}</span> : null}</> : access.canReadScoutDetails ? "Not disclosed" : <Link prefetch={false} href="/upgrade?tier=scout&source=tracker&content=launch-value&next=%2Ftracker%2Flaunches" className="font-bold text-potomac-gold underline-offset-4 hover:underline">Sign up or Log In for More Details</Link>}</dd></div>
                    </dl>
